...
//////////////////////////////////////////////////////////////////////////////
        ...
        void Update()
        {
            ...
            if (mouse_held_down && !no_drawing_on_current_drag && !EventSystem.current.IsPointerOverGameObject())
            {
        ...
        public void MarkPixelsToColour(Vector2 center_pixel, int pen_thickness, Color color_of_pen)
        {
            ...
            int index = 0;
            int lenghtX = (center_x + pen_thickness ) - (center_x - pen_thickness);
            int lenghtY = (center_y + pen_thickness) - (center_y - pen_thickness);

            float diameter = (center_x + pen_thickness) - (center_x - pen_thickness);
            ...
            for (float x = -diameter ; x <= diameter; x++)
            {
                ...
                {
                    index++;
                    float d = (x / radius) * (x / radius) + (y / radius) * (y / radius);
                    if(d < 1.2)//
                    {
                        int normalizedX = center_x - pen_thickness + (int)(x+radius);
                        int normalizedY = center_y - pen_thickness + (int)(y+radius);
                        MarkPixelToChange(normalizedX, normalizedY, color_of_pen);
                    }
                    if (d < 1.2)
                    {
                        int normalizedX = center_x - pen_thickness - (int)(x-radius);
